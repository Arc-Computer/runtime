...
pipeline_context: ContextVar[Optional[Dict[str, Any]]] = ContextVar('pipeline_context', default=None)
...
class MultiAgentContext:
    ...
    def __init__(self, pipeline_id: Optional[str] = None, application_id: Optional[str] = None):
    ...
    def get_pipeline_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_latency_ms': sum(agent.get('latency_ms', 0) for agent in self.agents),
            'agents': self.agents,
...
class AgentTracer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.start_time:
            ...
            logger.debug(f"Completed tracing agent: {self.agent_name} (latency: {latency_ms:.2f}ms)")
        return False
